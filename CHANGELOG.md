...
Per-release notes are also published on each [GitHub Release](https://github.com/gronke/rust-web_modules/releases) (sourced from the annotated tag) and on [crates.io](https://crates.io/crates/web_modules).
...
- The reusable **`web-modules build` GitHub Action** — a composite action that builds a deployable `dist/` (vendor npm, transform TS/SCSS, render `index.html` with the import map injected) with no Node on the runner.
  ...
  - A moving `v0` major tag, recreated by CI after each release to point at the highest stable 0.x, so `uses: gronke/rust-web_modules@v0` tracks the latest 0.x.
...
[0.3.0]: https://github.com/gronke/rust-web_modules/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/gronke/rust-web_modules/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/gronke/rust-web_modules/releases/tag/v0.1.0
